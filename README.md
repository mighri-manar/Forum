...
- **User Statistics**: Track posting history and activity metrics
...
```c
FORUM
├── list_of_authors[]        // Dynamic array of users
├── list_of_rubriques[]      // Dynamic array of rooms
├── list_of_profils[]        // User profile data
└── metadata (email, address, etc.)

AUTHOR
├── pseudo (username)
├── profil_author (PROFIL)
...
- **Dynamic Arrays**: Used for authors/rooms with realloc for scalability
- **Modular Design**: Separate .c/.h files for each struct (FORUM.c/h, AUTHOR.c/h, etc.)
...
gcc -o forum main.c FORUM.c AUTHOR.c PROFIL.c RUBRIQUE.c MESSAGE.c MENU.c -std=c99 -Wall
...
- Flag-based flow control (login state, delete account, etc.)
